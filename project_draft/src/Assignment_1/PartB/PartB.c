...
int main(void) {
    ...
    while (1) {
        // Main loop is idle, as button handlers manage transmissions
...
void bouton1_handler(void) {
    ...
    if (button1_press_count >= 1) {
        button1_last_time = button1_press_time;
    ...
    send_morse_code("I CAN MORSE");  // Send message once on button press
}
...
void bouton2_handler(void) {
    ...
    if (button1_press_count == 0) {
        interval = 0;  // Button1 has never been pressed
    } else if (button1_press_count == 1) {
        interval = button1_press_time;  // Time since boot
    } else {
        interval = button1_press_time - button1_last_time;  // Time between last two presses
    }

    send_number_as_morse(interval);  // Send interval in Morse code
}